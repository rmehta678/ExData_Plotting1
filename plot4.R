...
with(household_power_consumption, {plot(DateTime, Global_active_power, type = "n", xlab = "", ylab = "Global Active Power (kilowatts)" , axes = F, frame.plot=TRUE)
        axis(1, at=c(as.numeric(min(household_power_consumption$DateTime)), as.numeric(min(household_power_consumption$DateTime))+86400, as.numeric(min(household_power_consumption$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"))
        axis(2, yaxs = "r")
        ...
        axis(1, at=c(as.numeric(min(household_power_consumption$DateTime)), as.numeric(min(household_power_consumption$DateTime))+86400
                     , as.numeric(min(household_power_consumption$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"))
        axis(2, yaxs = "r")
        with(household_power_consumption, points(DateTime, Voltage, type = "l"))
        
        plot(DateTime, Global_reactive_power, type = "n", xlab = "datetime", ylab = "Global Reactive Power (kilowatts)" , axes = F, frame.plot=TRUE)
        axis(1, at=c(as.numeric(min(household_power_consumption$DateTime)), as.numeric(min(household_power_consumption$DateTime))+86400
                     , as.numeric(min(household_power_consumption$DateTime))+2*86400), labels=c("Thu", "Fri", "Sat"))
        axis(2, yaxs = "r")
        with(household_power_consumption, points(DateTime, Global_reactive_power, type = "l"))
        })
